...
public class AuthService(HttpClient httpClient, JsonSerializerOptions serializerOptions, IAppState appState, IPostApiResponseService postApiResponseService) : IAuthService
{
    ...
    ///     An instance of <see cref="LoginRequest" /> that contains the user's
    ...
    /// <summary>
    ///     Retrieves the authenticated user's data from the server and updates the application state with the user's
    ///     information.
    /// </summary>
    /// <remarks>
    ///     This method sends a request to the predefined user information API endpoint, processes the server's response,
    ///     and updates the application's state with the retrieved user details, if available. It is invoked after successful
    ///     user authentication.
    /// </remarks>
    /// <returns>
    ...
    private async Task<User?> ParseUserData(HttpResponseMessage response)
    {
        ...
        var isAdmin = attributes.GetProperty("is_admin").GetBoolean();

        return email != null && name != null
            ? new User(email, id, isEngineer, isAdmin, name)
            : null;
    }
}