...
///     It processes raw JSON data and converts it into strongly-typed ticket models with optional included data like
///     authors, engineers, and comments.
/// </summary>
public class TicketParser(
    IJsonParsingHelper jsonHelper,
    TicketParsingConfig config,
    IUserParser userParser,
    ICommentParser commentParser) : ITicketParser
{
    /// <summary>
    ...
    /// </remarks>
    private const string EngineerKey = "engineers";
    ...
    /// <returns>
    ///     An ObservableCollection of <see cref="Ticket" /> objects parsed from the JSON data. If parsing fails or no
    ///     tickets are found, an empty collection is returned.
    /// </returns>
    public async Task<ObservableCollection<Ticket>> ParseTickets(HttpResponseMessage response)
    {
        ...
        if (!rootElement.TryGetProperty(config.RootProperty, out var ticketArray) ||
            ticketArray.ValueKind != JsonValueKind.Array)
    ...
    /// <returns>
    ///     An instance of <see cref="TicketWithIncludes" /> containing the parsed ticket and its associated data,
    ...
    /// </summary>
    /// <param name="dataElement">
    ///     The JSON element containing ticket data, including the includes property which might hold the engineers to be
    ///     parsed.
    /// </param>
    /// <returns>
    ...
    /// <returns>
    ///     An observable collection of parsed comments as <see cref="Comment" /> objects. If no comments are found, an
    ...
    internal Ticket? ParseTicket(JsonElement ticketElement)
    {
        ...
        return new Ticket(
            id,
            jsonHelper.GetStringField(attributes, config.FieldMappings["error_code"]),
            ...
            jsonHelper.GetStringField(attributes, config.FieldMappings["reproduction_step"]),
            jsonHelper.GetDateTimeField(attributes, config.FieldMappings["created_at"]),
            jsonHelper.GetDateTimeField(attributes, config.FieldMappings["updated_at"])
        );
    }
}